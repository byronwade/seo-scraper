...
func extractContent(r *colly.Response) Content {
	plainText := extractText(string(r.Body))
	fullHTML := string(r.Body)
...
func extractPriorityIndicators(doc *goquery.Document, r *colly.Response) PriorityIndicators {
	return PriorityIndicators{
		...
		MediumPriority: extractMediumPriority(doc),
		LowPriority:    extractLowPriority(doc),
	}
}

func extractContentQuality(doc *goquery.Document) ContentQuality {
	// Extract main content
	content := extractText(doc.Find("body").Text())
	...
	return ContentQuality{
		WordCount:              wordCount,
		ContentUniquenessScore: calculateContentUniquenessScore(cleanContent),
...
func extractContentSections(doc *goquery.Document) []ContentSection {
	...
	doc.Find("h1, h2, h3, h4, h5, h6").Each(func(i int, s *goquery.Selection) {
		title := s.Text()
		text := extractText(s.NextUntil("h1, h2, h3, h4, h5, h6").Text())
...
func extractMediumPriority(doc *goquery.Document) MediumPriority {
...
func extractLowPriority(doc *goquery.Document) LowPriority {
...
func calculateContentUniquenessScore(text string) int {
...
func extractLSITerms(text string) []string {
...
func extractText(html string) string {
	...
	extractTextFromNode(doc.Find("body"), &textBuilder)
...
func extractTextFromNode(s *goquery.Selection, textBuilder *strings.Builder) {
	if s.Is("script, style, noscript, iframe, svg") {
		return
	}
	...
	s.Contents().Each(func(i int, s *goquery.Selection) {
		extractTextFromNode(s, textBuilder)
	})
}
